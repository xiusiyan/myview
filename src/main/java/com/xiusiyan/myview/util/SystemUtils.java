...
import com.xiusiyan.myview.DefaultException;
...
public class SystemUtils extends org.apache.commons.lang.SystemUtils {
        ...
        if (environment == null) {
            try {
                if (SystemUtils.IS_JAVA_1_4) {
                    ...
                    Method envMethod = target
                            .getMethod("getenv", ArrayUtils.EMPTY_CLASS_ARRAY);
                    environment = Collections.unmodifiableMap((Map) envMethod
            ...
            while ((line = br.readLine()) != null) {
                for (int prefix = 0; prefix < UNIX_ENV_PREFIXES.length; prefix++) {
                    if (line.startsWith(UNIX_ENV_PREFIXES[prefix])) {
                        line = line.substring(UNIX_ENV_PREFIXES[prefix]
                                .length());
                    }
                }
    ...
    private static CommandLine parseCommandLine(String args[], String opts[][])
            throws DefaultException {
        Options options = new Options();
        for (int i = 0; i < opts.length; i++) {
            options.addOption(opts[i][0], opts[i][1].equals("true") ? true
                    : false, opts[i][2]);
        }
        ...
        try {
            ...
            if (line == null) {
                throw new DefaultException(
                        "Unknown error parsing the Mule command line");
            }

            return line;
        } catch (ParseException p) {
            throw new DefaultException(
                    "Unable to parse the Mule command line because of: "
                            + p.toString(), p);
        }
    }
    ...
    public static String getCommandLineOption(String option, String args[],
            String opts[][]) throws DefaultException {
     ...
     */
    // TODO MULE-1947 Command-line arguments should be handled exclusively by the bootloader
    public static boolean hasCommandLineOption(String option, String args[],
            String opts[][]) throws DefaultException {
        CommandLine line = parseCommandLine(args, opts);
     ...
     */
    // TODO MULE-1947 Command-line arguments should be handled exclusively by the bootloader
    public static Map getCommandLineOptions(String args[], String opts[][])
            throws DefaultException {
        CommandLine line = parseCommandLine(args, opts);
        Map ret = new HashMap();
